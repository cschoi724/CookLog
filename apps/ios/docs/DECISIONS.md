...
- 상태: 확정
- 결정: 2026-06-19~22의 Mock Core MVP 범위 결정은 초기 구현 이력으로 보존하고, 현재 제품 범위는 루트 `CookLog_MVP_SCOPE.md`, PRD와 실행 Task를 따릅니다.
- 이유: 로컬 검색, 진행 기록 저장과 첫 공개 출시 핸즈프리가 추가 확정돼 과거 제외 결정을 현재 출시 범위로 해석하면 안 됩니다.
- 영향: iOS Agent는 아래 과거 결정을 현재 출시 제외 근거로 사용하지 않습니다.
- 후속 작업: T-20260731-001에서 활성 iOS 안내·계획·스펙·서비스 문서를 최신 참조 구조로 갱신합니다.
...
- 상태: TTS 결정 유지, 음성 명령 범위는 첫 공개 출시 기준으로 대체됨
- 결정: 초기 오디오 가이드는 `AVSpeechSynthesizer` 기반 TTS로 구현합니다.
- 이유: 저장된 조리 단계를 바로 읽을 수 있고, 별도 오디오 파일 생성이나 서버 처리가 필요 없습니다.
- 영향: 녹음 파일 저장과 고품질 음성 합성은 후속으로 미룹니다. 핸즈프리 음성 명령은 최신 제품 결정에 따라 첫 공개 출시에서 지원합니다.
...
- 상태: 확정
- 결정: iOS 제품 기준은 최신 `docs/product/CookLog_PRD_v2.md`를 사용합니다. `CookLog PRD v2.pdf`는 2026-06-22 역사적 스냅샷입니다.
...
- 상태: Core MVP 이력, 첫 공개 출시 기준으로 대체됨
...
- 후속 작업: 첫 공개 출시에는 최신 제품 결정에 따라 제목·재료명 로컬 검색을 제공합니다.
