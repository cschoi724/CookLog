...
pr:
  url:
  status:
blocker: T-20260813-003과 화면별 집중 시각 보강 T-20260813-005~011의 완료가 필요하다.
...
- 7개 화면군·82개 상태 추적 매트릭스와 Home 정확 일치 기준·화면별 대표 Light 승인·전체 Light/Dark 시각 승인 사실을 민감 식별자 없이 실행 보고서에 정리한다.
...
- `390×844 Light/Dark`에서 고정 Home 기준과 화면별 승인 결과, 팝 키치·힙 시각 언어와 semantic state의 일관성을 확인한다.
...
1. Product Owner의 Home 기준 화면 정확 일치 승인, 나머지 6개 화면군 대표 Light 방향 승인 및 전체 390×844 Light/Dark 승인 사실이 식별자 없이 추적된다.
...
8. Home은 고정 기준과 geometry·spacing·typography·copy·color·component hierarchy가 측정 가능하게 일치하고, 나머지 화면은 승인된 화면별 위계와 팝 키치 컨셉을 유지하며 기존 Prototype 레이아웃의 단순 재색칠로 회귀하지 않는다.

## Coordination Notes

- T-20260813-011이 `done`이 되기 전 실행하지 않는다.
...
T-20260813-003과 T-20260813-005~011이 완료되고 Product Owner가 이 Task를 승인한 뒤 T-20260813-004를 실행해줘.
