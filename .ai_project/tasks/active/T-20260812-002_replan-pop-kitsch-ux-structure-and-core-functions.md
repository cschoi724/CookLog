...
approved_by: Product Owner (2026-08-12, UX 구조·핵심 기능 재기획 실행 승인)
locked_by:
locked_at:
lock_session:
...
- Acceptance criteria:
  ...
  - 390×844pt·375×667pt에서의 화면별 콘텐츠 블록·레이아웃·CTA·상호작용 원칙, 긴 설명 축소와 용어 단순화 원칙이 명시된다.
  - Design Lead가 이를 근거로 T-005~007의 UI/UX 원본 적용 범위·의존성을 재조율할 수 있는 화면별 handoff가 제공된다.
...
- Validation required: Product QA가 기존 기능 분류, 제품 문서와 상태·기능 계약의 모순, 화면별 우선순위·레이아웃·상호작용 누락, 승인되지 않은 기능 확장을 독립 검토한다.

이 Task의 범위·소유권·산출물·후속 인계 조건은 확정됐다. Product Owner 실행 승인 후 Product Planning Agent가 실행하며, 이후 Design Lead Agent가 자신의 Team 소유 T-005~007의 dependency와 scope를 이 기획 결과에 맞춰 조정한다.
