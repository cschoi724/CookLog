...
Task T-20260805-008은 승인된 실행 Task야.

- 현재 상태: `approved`
- 기준 상태 ref: `origin/develop`
- 기준 상태 SHA: `56cb68d`
- 다음에 해야 할 일: 최신 canonical 기준 전용 worktree에서 lock을 획득하고, 허용 경로
  안에서 접근성·작은 화면·다크 모드·통합 회귀 fixture와 증빙을 완성해. 발견된 iOS 결함은
  범위 안에서 수정하고 자체 검증 후 task report를 작성해.
- 기준 문서: Task `source_of_truth` 전체
- 허용 경로: Task frontmatter의 `allowed_paths`
- 참고 산출물: `.ai_project/tasks/active/T-20260805-008_ios-accessibility-visual-regression.md`
- 변경/검토 대상: `apps/ios/`, Core Loop 23개, 통합 82개 상태, 390×844·375×667
  Light/Dark·기본/Accessibility 3, Dynamic Type·VoiceOver·44pt
- 남은 리스크: 실제 Apple STT·Backend AI·TTS 엔진과 운영 문의·법적 값은 후속 범위
- 차단/결정 필요: 허용 경로 밖 수정이나 Source of Truth 충돌이 발견되면 중단하고
  Development Lead Agent에 보고
- 완료 시: report를 갱신하고 lock을 해제한 뒤 `verification_ready`로 전환해 iOS QA
  Agent / Verification Role에 독립 검증을 요청해.
