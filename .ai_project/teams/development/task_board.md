...
`T-20260728-004`는 iOS QA 독립 재현, Product Owner 위험 수용과 PR #8 squash merge를 마쳐 `done`입니다. Xcode·Simulator 고정 검증은 T-008로 인계했지만 별도 승인 전 자동 시작하지 않습니다. 일반 개발 Task는 최신 `develop` 기반 전용 worktree와 `develop` 대상 PR을 사용합니다. proposed Task는 Lead scope와 Product Owner 승인 전 실행하지 않습니다.
