...
| `T-20260728-003` | `scoped` | iOS | 확정 제품 UX·디자인과 iOS 로컬 상태 모델 적용 | Development Lead Agent | `T-20260812-003`, 하위 `T-20260805-002~008`, `T-20260812-001` | Figma baseline → iOS 일괄 동기화·기능 QA → Visual Design QA 후 완료 리뷰 |
...
| `T-20260805-008` | `blocked` | iOS | UI 구현·기능·기술 접근성 통합 검증 | Development Lead Agent / Lead Role | `T-20260812-003` Figma baseline 고정 대기 | 기존 WIP·실패 이력 보존, Figma 차이 일괄 동기화 후 재개 승인; Visual QA는 T-20260812-001 |
...
Backend QA 재검증에서 HIGH 2건 해소와 원본 공격 반례, T-006 26/26·전체 92/92·공용
계약 무회귀를 확인했습니다. Development Lead가 허용 경로와 성공 기준을 직접 재검토해
...
Product Owner가 `T-20260804-007` Backend Foundation 최종 통합·보안 검증과 로컬 실행
...
Product Owner가 `QA-HIGH-007-001` 재작업을 승인했습니다. Backend Agent는 terminal
감사 성공 전 성공 결과를 외부 공개하지 않도록 fail closed하고, sink 장애·reservation
거절·event shape 거절과 재시도 Provider at-most-once 회귀를 추가합니다. 통과한 96/96,
계약 validator 5종과 container 경계는 유지합니다.

Backend QA 재검증에서 원본 반례와 terminal sink·reservation·shape 장애가 모두
비공개 staging 후 복구되고 Provider 호출이 1회로 유지됨을 확인했습니다. 전체 100/100,
...
Development Lead가 최신 `origin/develop@04aa1bc` 통합 상태에서 허용 경로, 독립 QA
62/62, 색상 토큰 증빙과 PR #90의 iOS build·XCTest checks 통과를 확인했습니다. 완료
리뷰는 `PASS_WITH_RISK`, 상태는 `completion_review`이며 Product Owner의 완료·병합
승인 전에는 Draft PR을 병합하지 않고 T-005 차단을 유지합니다.
...
Product Owner가 동일 record ID와 완료 Recipe 편집·삭제 조립에 필요한
`AppEnvironment.swift`, `AppRoute.swift`, `CookLogApp.swift`의 최소 경로 확장을
승인했습니다. iOS Agent는 `origin/develop@493743e` 전용 worktree에서 lock을 획득하고
T-005를 `in_progress`로 전환했습니다.

iOS Agent가 Review 5개 상태, 같은 UUID·STEP snapshot, 임시 저장·이탈 복원, 완료 Recipe
수정·영구 삭제와 실패 보존을 구현했습니다. 전체 XCTest 72/72·build와 iPhone 15
Light/Dark Review를 확인해 `verification_ready`로 iOS QA Agent에 인계했습니다.

iOS QA와 Development Lead가 전체 XCTest 72/72, PR #94 iOS build·XCTest와 잔여 위험을
...
iOS Agent가 구현 commit `8d0321e`에서 집중 13/13·전체 XCTest 77/77과 4개 viewport를
제출했으나 iOS QA 독립 검증에서 375×667 Light/Dark의 `재료 알려줘` CTA가 하단 고정
control bar에 가려지는 `QA-HIGH-806006-001`을 확인했습니다. Product Owner가 하단
safe-area/content inset 한정 재작업을 승인했으며 기존 Player 상태·action·보존 로직은
유지하고 같은 QA Agent에게 재검증합니다. T-007은 계속 차단합니다.
...
accent·success·error를 확정 Light/Dark 토큰으로 교체합니다. 전체 XCTest와
`LOG-STEP-ADDED`, `LOG-ERROR` Light/Dark 증빙 후 iOS QA에 독립 재검증을 요청합니다.
