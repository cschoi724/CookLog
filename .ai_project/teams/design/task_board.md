...
`T-20260729-008`은 완료 카드 badge 제거와 Gallery variant 보완에 대한 독립 재검증 및 Design Lead 완료 검토를 통과하고 PR #11로 `develop`에 squash merge되어 `done`으로 확정했습니다. `T-20260729-009`는 선행 의존성을 충족했지만 별도 Product Owner 실행 승인 전까지 `proposed`를 유지하며, `T-20260729-010~014`도 순차 선행 Task 완료와 별도 승인 전까지 시작하지 않습니다.
