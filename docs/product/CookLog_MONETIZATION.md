...
| 단계 | Task ID | 실행 조건 |
|---|---|---|
| 정책 | `T-20260728-010` | Backend foundation과 실서비스 준비도 완료 |
| Design | `T-20260728-011` | MVP Figma와 가격·기능 정책 승인 |
...
모든 Task는 초기 등록 시 `proposed`로 유지합니다. `depends_on`이 완료되고 현재 제품 Queue에서 실서비스 수익화의 우선순위가 도래했을 때만 각 Lead가 `scoped`로 전환하며, Product Owner 승인 전에는 실행하지 않습니다.
