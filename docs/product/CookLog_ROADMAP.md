...
실제 iPhone에서 사용자가 요리 중 10초 음성 기록을 반복하고, Apple 기기 내 STT와 온라인 AI로 개인 레시피를 만든 뒤, 저장된 레시피를 버튼과 핸즈프리 Audio Guide로 다시 요리할 수 있는 iOS 앱을 공개 출시합니다.

Mission Clear 조건:

- Mock이 아닌 실제 Apple 기기 내 STT와 Backend AI 정리가 동작합니다.
...
- Backend Contract: `T-20260728-005` AI gateway와 기본 비활성 원격 STT adapter 계약
- Backend Foundation: `T-20260728-006` Mock AI provider와 선택형 원격 STT adapter 기반 Backend foundation
...
- AI 정리의 버전된 계약과 원격 STT adapter의 비활성 경계, 개인정보·idempotency·오류 기준이 있습니다.
...
- Backend Production: `T-20260729-003` 실제 AI provider와 배포 가능한 Backend gateway, 기본 비활성 원격 STT adapter
- iOS On-device STT: `T-20260729-004` 10초 녹음·권한·Apple 기기 내 STT 연동
...
- 기기 내 STT와 온라인 AI 실패가 기존 로컬 기록을 손상하지 않습니다.
- 기기 내 STT 실패가 원격 STT 자동 fallback이나 무승인 음성 전송을 일으키지 않습니다.
...
- 실제 지원 iPhone에서 기기 내 STT가 동작하고 개발·스테이징 환경의 AI와 iOS 전체 흐름이 연결됩니다.
...
│   │                                      ├─> T-20260729-005
│   │                                      └─> T-20260729-006
│   └─> T-20260728-005 ─> T-20260728-006 ─> T-20260729-003 ─> T-20260729-005
└─> T-20260728-004 ─> T-20260728-008

R1·R2 차단 Task 전체 ─> T-20260728-009
```

`T-20260729-004`는 `T-20260729-026` 정책과 iOS 로컬 제품 기준 이후 시작하며 Backend STT 환경을 기다리지 않습니다. `T-20260729-005`는 실제 서비스 중복 재작업을 피하기 위해 `T-20260729-003`의 검증된 개발·스테이징 AI 환경 이후 시작합니다.
...
| `T-20260728-005` | 범위 수정·P0 | 온라인 AI 계약을 출시 필수로 하고 원격 STT는 기본 비활성 adapter 계약으로 분리합니다. |
| `T-20260728-006` | 범위 수정·P0 | 실제 AI provider 전 Mock 기반 계약·보안 foundation이 필요하며 원격 STT는 비활성 확장 경계만 둡니다. |
...
1. 실제 기기 내 STT·온라인 AI 성공률, 지연시간과 AI 비용 확인
2. 기기 내 STT 품질·지원 범위가 부족하거나 실제 수요가 확인되면 원격 STT adapter 활성화를 별도 평가
3. 기록 완료율, 저장 완료율, Audio Guide 사용과 재사용률 확인
4. 개인정보·장애 문의와 데이터 복구 요구 확인
5. 근거가 확보되면 수익화 Workstream 재개
6. 이후 Import, 백업·동기화와 Android 순서를 다시 결정
