...
- 상태: 확정
- 결정: `RecipeAIService` 인터페이스와 Mock 구현을 먼저 만들고, 실제 AI API 연동은 전체 앱 흐름이 동작한 뒤 검토합니다.
- 이유: CookLog의 핵심 검증은 API 연동 자체가 아니라 기록이 레시피가 되고 다시 요리할 수 있는 흐름입니다.
- 영향: 초기 구현에서는 외부 네트워크와 API 키 없이 개발할 수 있습니다.
- 후속 작업: Mock AI 정리 규칙을 만들고, AI Review 화면 저장 흐름을 먼저 완성합니다.
